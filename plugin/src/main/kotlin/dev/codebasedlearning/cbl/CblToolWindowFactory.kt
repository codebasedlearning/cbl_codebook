...
class CblPanel(private val project: Project) : JPanel(BorderLayout()) {
    ...
    private fun updateModel() {
        ...
        // a re-parse (debounced doc change) must not wipe the selection -
        // essential with follow-caret off, where it is the only state there is;
        // identity across re-parses is the title (same convention as fold state)
        val previousTitle = tocList.selectedValue?.title
        syncingSelection = true
        try {
            tocModel.replaceAll(model?.blocks ?: emptyList())
            val restored = model?.blocks?.firstOrNull { it.title == previousTitle }
            if (restored != null) {
                tocList.setSelectedValue(restored, true)
                showBlock(restored)
            } else {
                notesPane.text = ""
            }
    ...
    private fun showBlock(detail: CblBlock, preserveScroll: Boolean = false) {
        ...
        // single header: the full breadcrumb (all titles, one line, normal
        // title color) - the bodies below carry no headers of their own.
        // Each crumb carries its own level emphasis, exactly like the TOC:
        // bold topic, italic child, plain below - so no outer <b> here.
        val crumbs = layers.joinToString(" &#9656; ") { levelEmphasis(inlineTitle(it), it.depth) }
        val html = StringBuilder("<html><body style='margin:6px 8px'>")
        html.append(crumbs).append("<hr>")
        ...
        for (block in layers) {
            ...
            val body = renderBody(block)
            if (first) html.append(body)
            else html.append("<div class='detail'>").append(body).append("</div>")
            first = false
        }
    ...
    private inner class WrappingRenderer : JPanel(BorderLayout()), ListCellRenderer<CblBlock> {
        ...
        override fun getListCellRendererComponent(
            ...
            bullet.border = JBUI.Borders.empty(2, 6 + (value.depth - 1) * 14, 2, 0)
            title.text =
